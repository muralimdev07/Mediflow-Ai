...
@router.get("/me", response_model=SuccessResponse[Optional[QueueEntryRead]])
def get_my_queue(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Get current patient's queue position."""
    service = QueueService(db)
    entry = service.get_patient_active_queue(current_user.id)
